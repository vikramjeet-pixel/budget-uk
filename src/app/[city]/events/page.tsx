...
import { EVENTS_CONTENT } from "@/data/events-content";
...
export async function generateMetadata({ params }: { params: Promise<{ city: string }> }): Promise<Metadata> {
  ...
  return {
    title: `${cityName} Tech Events`,
    description: `A hand-curated guide to recurring tech events, founder meetups, and communities in ${cityName}.`,
...
export default async function EventsPage({ params }: { params: Promise<{ city: string }> }) {
  const { city } = await params;
  const content = EVENTS_CONTENT[city.toLowerCase()];
  const cityName = CITIES.find(c => c.slug === city.toLowerCase())?.name || city;
    ...
    <div className="min-h-screen bg-[#f7f4ed]">
      <main className="max-w-3xl mx-auto px-4 py-16 md:py-24 mt-4 flex flex-col gap-16">
        <header className="flex flex-col gap-4">
          <h1 className="t-h1 text-[#1c1c1c] tracking-tighter">{content.title}</h1>
        ...
        <section className="flex flex-col gap-8">
          <div className="flex flex-col gap-2">
            <h2 className="t-h2 text-[#1c1c1c] tracking-tight">
              People to follow for {cityName} tech
            </h2>
            <p className="t-body text-[#5f5f5d]">
              These are the people who announce things first — upcoming events, new cohorts, and ecosystem news.
            </p>
          </div>
