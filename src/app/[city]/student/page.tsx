...
import { STUDENT_CONTENT } from "@/data/student-content";
...
export async function generateMetadata({ params }: { params: Promise<{ city: string }> }): Promise<Metadata> {
  ...
  return {
    title: `Student ${cityName} — Budget Guide`,
    description: `Banking, phone plans, discounts, transport, housing, and food tips for students in ${cityName}.`,
    alternates: { canonical: `/${city}/student` },
  };
}
...
function AffiliateCTABlock({ cta }: { cta: AffiliateCta }) {
  return (
    <aside
      ...
      className="relative bg-[#1c1c1c] rounded-xl px-5 py-5 flex flex-col sm:flex-row items-center gap-4"
    >
...
function SectionNav({ sections }: { sections: StudentSection[] }) {
  return (
...
export default async function StudentPage({ params }: { params: Promise<{ city: string }> }) {
  const { city } = await params;
  const content = STUDENT_CONTENT[city.toLowerCase()];
  const cityName = CITIES.find(c => c.slug === city.toLowerCase())?.name || city;
