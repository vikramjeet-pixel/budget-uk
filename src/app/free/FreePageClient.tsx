    ...
    <section className="flex flex-col gap-5">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="flex flex-col gap-4">
          {withHours.map((spot) => (
            <SpotWithStatus key={spot.id} spot={spot} showStatus={openNow} />
          ))}
...
function SpotWithStatus({ spot, showStatus }: { spot: Spot; showStatus: boolean }) {
  const open = useMemo(() => {
...
export function FreePageClient({ spots }: { spots: Spot[] }) {
      ...
      <main className="max-w-3xl mx-auto px-4 py-16 md:py-24 mt-4 flex flex-col gap-12">
        ...
        <header className="flex flex-col gap-5">
          ...
          <p className="t-body-lg text-[#5f5f5d] max-w-xl leading-relaxed">
            Museums, parks, markets, concerts, and walks that cost nothing. London has more free world-class culture than almost any city on earth — most visitors don't realise how much.
          </p>
        ...
        {/* Community spots */}
        <CommunitySpots spots={spots} openNow={openNow} />
