...
export function SpotsGridContent({ citySlug }: { citySlug: string }) {
  ...
  const filteredSpots = useMemo(() => {
    ...
    return rawSpots.filter(spot => 
      spot.name.toLowerCase().includes(query) || 
      spot.description.toLowerCase().includes(query) ||
      spot.postcodeDistrict?.toLowerCase() === query
    );
    ...
    <div className="min-h-screen flex flex-col">
      <div className="sticky top-18 z-30 bg-[#fcfbf8]/95 backdrop-blur-md border-b border-passive px-4 py-3 flex flex-col gap-3 shadow-sm">
        <div className="flex gap-2 items-center">
          ...
          <SortDropdown isNearestAvailable={userLocation !== null} onNearestRequest={requestLocation} />
        </div>
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-3">
          <div className="flex flex-wrap gap-2">
            <NeighbourhoodFilter citySlug={citySlug} />
            <PriceDietaryFilter />
          </div>
          ...
          <span className="t-caption text-[#5f5f5d]">
            {loading
              ? "Loading spots…"
              : `${rawSpots.length} spot${rawSpots.length === 1 ? "" : "s"} in ${city?.name} · ${filteredSpots.length} match`}
          </span>
        </div>
      </div>
      ...
      <main className="grow p-4 md:p-6 lg:p-8">
        {loading ? (
          ...
          <div className="w-full py-20 text-center text-[#5f5f5d]">
            No spots match your filters in {city?.name}.
          </div>
        ) : (
