...
export function NeighbourhoodFilter({ citySlug = "london" }: NeighbourhoodFilterProps) {
          ...
          >
            <div className="flex flex-col gap-6">
              <div className="flex items-center justify-between">
                <span className="font-semibold text-[#1c1c1c]">Areas in {city?.name}</span>
              ...
              <div className="space-y-6">
                {isLondon ? (
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {city?.neighbourhoods.map((nbh) => (
                      <button
                        key={nbh}
                        onClick={() => toggleNbh(nbh)}
                        className="flex items-center gap-3 py-2 px-3 rounded-lg hover:bg-passive transition-colors text-left group"
                      >
                        <div className={cn(
                          ...
                          {selectedNbhs.includes(nbh) && <Check className="w-3.5 h-3.5 text-white" />}
                        </div>
                        <span className={cn(
                          "text-[14px] transition-colors",
                          selectedNbhs.includes(nbh) ? "text-[#1c1c1c] font-bold" : "text-[#5f5f5d] group-hover:text-[#1c1c1c]"
                        )}>
                          {nbh}
                        </span>
                      </button>
